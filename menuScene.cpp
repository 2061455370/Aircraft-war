...
void menuSceneDraw(struct menuScene* s)
{
    ...
    if (s->isStartGameHover == true)
        ...
        settextcolor(BLACK);
    drawtext("¿ªÊ¼ÓÎÏ·", &s->rectStarGame, DT_CENTER);
...
void menuSceneControl(struct menuScene* s, ExMessage* msg)
{
    ...
    case WM_MOUSEMOVE:
    {
        if (msg->x > s->rectStarGame.left && msg->x < s->rectStarGame.right
            && msg->y < s->rectStarGame.bottom && msg->y > s->rectStarGame.top)
...
void menuSceneInit(struct menuScene* s)
{
    ...
    s->bk = new IMAGE;
    loadimage(s->bk, "asset/img/menu.png");

    s->rectStarGame.left = 130;
    s->rectStarGame.top = 430;
    s->rectStarGame.right = 300;
    s->rectStarGame.bottom = 480;
